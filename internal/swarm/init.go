// Package swarm implements multi-agent coordination for parallel task execution.
//
// The swarm package provides session initialization, task decomposition (prompt
...
// Init initializes a swarm session by recording an event and returning session info.
...
func Init(store *db.Store, projectPath, isolation string) (map[string]any, error) {
	...
	_, err = store.DB.Exec(
		...
		"swarm_init", string(payloadJSON), projectPath,
	)
	if err != nil {
		return nil, fmt.Errorf("record swarm_init event: %w", err)
	}
