...
func TestGetStrategyInsights_WithData(t *testing.T) {
	...
	for _, o := range outcomes {
		...
		store.DB.Exec("INSERT INTO events (type, payload) VALUES (?, ?)", "swarm_outcome", string(payload))
	}
...
func TestGetFileInsights_WithData(t *testing.T) {
	...
	for _, p := range payloads {
		...
		store.DB.Exec("INSERT INTO events (type, payload) VALUES (?, ?)", "swarm_outcome", string(data))
	}
